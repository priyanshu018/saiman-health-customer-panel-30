...
> **2026-08-02 update (7):** a full engineering production-readiness audit (security, error handling, performance, Next.js best practices, accessibility, code quality — not visual/UX) found and fixed a real price-tampering hole in `/api/payments/create-order` (doctor and pharmacy checkout trusted a client-sent amount; both now re-price server-side like the other four service types, and the route now rejects any priced-service request missing a `bookingRef`), an unauthenticated PII leak plus open redirect on the payment checkout page, two cross-account payment-ownership gaps in `verify`/`link-booking`, and a "pay for a cheap service, get an expensive one marked paid" gap in `fulfill` for hospital/ctmri/rental bookings (not yet closed for lab bookings specifically). Also added the app's first error boundaries (`app/error.tsx`, `app/global-error.tsx` — there were previously none), fixed 11 unhandled promise rejections and replaced 7 of 8 `alert()` calls with inline error UI, deleted ~1000 lines of confirmed dead code, added per-route metadata/robots.txt/sitemap.ts, memoized 8 unmemoized search-filter computations, and made several accessibility fixes (aria-labels, label/input pairing on login/signup, fixed invalid nested-interactive markup). No redesign, new features, or workflow changes — see `APP_PROCESS_LOG.md`'s "Final Phase" entry for the full list and an honest remaining-work accounting.
>
> **2026-08-02 update (6):** a full browser-based visual QA pass (Playwright, desktop + mobile viewports, all 32 routes + 5 detail-page types) found and fixed several real bugs beyond styling: an infinite loading spinner for logged-out visitors on 5 booking-history screens (`if (!user) return` left `loading` stuck `true` forever), 8 more instances of fixed-column CSS grids breaking on mobile (React inline styles can't hold media queries — Subscription Plans and the doctor detail page were both unreadable/overlapping on a 390px viewport before this), a currency-formatting bug showing ragged single-decimal prices, a white-on-white empty-state CTA button across 12 screens, and misleading fallback copy ("Hospital: Online consultation" when no hospital was set). No new features/routes were added — this was scope-limited to fixing genuine defects. See `APP_PROCESS_LOG.md`'s "Phase 5" entry for the full list.
>
...
>
> **2026-08-01 update (5):** a visual/UX polish pass upgraded the shared card/button/hero/form/empty-state styling used across every module page, added real hover interactions, and rebuilt the payment confirmation experience. It also found and fixed a real bug: the fully-built customer dashboard (`WebHomeScreen`) was dead code — no route rendered it, so `/` always showed the public marketing page even when logged in. That's fixed. Not done: bespoke per-service hero layouts/illustrations, Profile page feature depth (family/documents/saved providers/payment history), and a full accessibility/performance audit. See `APP_PROCESS_LOG.md`'s "Phase 4" entry for exact scope.
>
> **2026-08-01 update (4):** §6 (Lab Tests), §7 (CT/MRI), §8 (Hospitals & Surgery), and §10 (Rental Equipment) below are now **out of date** — all four were taken from browse-only to complete transactional flows (detail page, booking form, server-side priced Razorpay payment, durable backend row, history page). Hospitals now says "Request consultation" instead of the inaccurate "Instant booking." A new shared, idempotent fulfillment endpoint (`/api/service-bookings/fulfill`) backs all four, with server-side price recomputation added to `/api/payments/create-order` (additive — doctor/pharmacy behavior unchanged). Not done: file uploads, realtime status updates, and multi-item lab cart for any of the four. See `APP_PROCESS_LOG.md`'s "Phase 3" entry and completion matrix for exact scope.
>
